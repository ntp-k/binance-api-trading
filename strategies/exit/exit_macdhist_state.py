...
class ExitMacdHistState(BaseExitStrategy):
    ...
    def __init__(self, bot_config: BotConfig, logger=None):
        ...
        self.macd_decimal = int(self.dynamic_config.get('macd_decimal', 2))
        self.close_price_diff_threshold = float(self.dynamic_config.get('close_price_diff_thsd', 0.0))
    ...
    def should_close(self, klines_df, position_handler) -> PositionSignal:
        ...
        if histogram_against_position:
            checklist_reasons.append(f"Hist {current_hist} against {position.position_side.name}: ✅")
        else:
            checklist_reasons.append(f"Hist {current_hist} against {position.position_side.name}: ❌")
